...
class StreamDeckApi:
    ...
    async def get_info(self, in_executor: bool = True) -> bool | SDInfo:
        """Get info about Stream Deck."""
        res: requests.Response | bool = False
        if in_executor:
            res = await self._loop.run_in_executor(
                None, self.get_request, self.info_url
            )
        else:
            res = self.get_request(self.info_url)
    ...
    async def get_icon(self, btn: str) -> bool | str:
        ...
        res = await self._loop.run_in_executor(None, self.get_request, url)
        if isinstance(res, bool) or res.status_code != 200:
            return False
